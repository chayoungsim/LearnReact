 ...
 */
export function renderRegularDays(year, month) {
 ...
 */
export function updateRegularTitle(year, month) {
 ...
 */
export function changeRegularMonth(delta) {
 ...
 */
export function setRegularYearMonth(year, month) {
...
/**
 * Regular Calendar 현재 날짜 상태 가져오기
 */
export function getRegularCurrentDate() {
    return { ...regularCurrentDate };
}

/**
 * Regular Calendar 초기화
 */
export function initRegularCalendar() {
    // dayjs 플러그인 초기화 확인
    ...
    if (prevBtn) {
        prevBtn.addEventListener('click', () => changeRegularMonth(-1));
    }
    
    if (nextBtn) {
        nextBtn.addEventListener('click', () => changeRegularMonth(1));
    }
    ...
    // 초기 렌더링
    renderRegularDays(regularCurrentDate.year, regularCurrentDate.month);
}

/**
 * Regular Calendar 파괴 (이벤트 리스너 제거)
 */
export function destroyRegularCalendar() {
    const prevBtn = document.getElementById('regularPrevBtn');
    const nextBtn = document.getElementById('regularNextBtn');
    const regularTitle = document.getElementById('regularTitle');
    ...
    if (regularTitle) {
        const newTitle = regularTitle.cloneNode(true);
        regularTitle.parentNode.replaceChild(newTitle, regularTitle);
    }
