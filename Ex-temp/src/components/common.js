...
function headerAction() {
  ...
  const lastFamilySiteLink = document.querySelector('.family-site-list-last');
  if (lastFamilySiteLink) {
    lastFamilySiteLink.addEventListener('keydown', function(e) {
      if (e.key === 'Tab' && !e.shiftKey) {
          e.preventDefault();
          document.querySelector('.btn-navbar')?.focus();
          document.querySelector('.sitemap-util .family-site').classList.remove('open');
       }
    });
  }

   const lastEngLink = document.querySelector('.sitemap-util .btn-outline');
   if(lastEngLink) {
     lastEngLink.addEventListener('keydown', function(e) {
       if (e.key === 'Tab' && !e.shiftKey) {
           e.preventDefault();
           document.querySelector('.btn-navbar')?.focus();
        }
  ...
  if (linkOpenFamily) {
    if (window.location.pathname.indexOf('/kor/') > -1) {
      linkOpenFamily.addEventListener('click', () => {        
         linkOpenFamily.parentElement.nextElementSibling.classList.remove('on');
         linkOpenFamily.parentElement.classList.toggle('on');
       });
    } else {
      if (linkOpenFamily.parentElement) {
        linkOpenFamily.parentElement.style.display = 'none';
      }
    }
  }

  if (linkOpenLanguage && popLanguage) {
    linkOpenLanguage.addEventListener('click', () => {   
       linkOpenLanguage.parentElement.previousElementSibling.classList.remove('on');
       linkOpenLanguage.parentElement.classList.toggle('on');
     });
  }
...
function initNavbarSitemap() {
  const sitemap = document.querySelector('.navbar-sitemap');
  if (!sitemap) return;

  const sitemapBtn = sitemap.querySelector('.family-site button');
  if(!sitemapBtn) return;

  sitemapBtn.addEventListener('focus click',(e) =>{
      e.currentTarget.closest('.family-site').classList.add('open');
  })
}
